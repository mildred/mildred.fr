...
#   2.) Change config_root_url to reflect your domain name.
...
#       @custom_var[:change_frequency] and @custom_var[:priority]
#       constants.
# 
# Notes:
...
module Jekyll
  ...
  class SitemapGenerator < Generator
    ...
    def generate(site)
      ...
      file = File.new(File.join(site.dest, sitemap_xml), "w")
      ...
      site.static_files << Jekyll::SitemapFile.new(site, site.dest, "/", sitemap_xml)
    end
    ...
    def fill_url(site, page_or_post)
      ...
      if (page_or_post.data[@custom_var[:change_frequency]])
        change_frequency = 
          page_or_post.data[@custom_var[:change_frequency]].downcase
      ...
      if (page_or_post.data[@custom_var[:priority]])
        priority_value = page_or_post.data[@custom_var[:priority]]
        if valid_priority?(priority_value)
          priority = REXML::Element.new "priority"
          priority.text = page_or_post.data[@custom_var[:priority]]
